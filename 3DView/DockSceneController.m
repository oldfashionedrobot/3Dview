...
#import "DockSceneController.h"

@interface DockSceneController ()
...
@implementation DockSceneController

SCNNode *cameraNode;
double pitchRestricts = 26;
...
- (IBAction)scrolly:(UIPanGestureRecognizer *)recognizer {
    ...
    if( RADIANS_TO_DEGREES(pitch) > pitchRestricts ) pitch = DEGREES_TO_RADIANS( pitchRestricts );
    else if( RADIANS_TO_DEGREES(pitch) < -pitchRestricts ) pitch = DEGREES_TO_RADIANS( -pitchRestricts );
...
- (void)viewDidLoad {
    ...
    // Add sphere thing
    SCNScene *geo = [SCNScene sceneNamed:@"dockScene.dae"];
