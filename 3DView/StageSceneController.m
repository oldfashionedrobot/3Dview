...
#import "StageSceneController.h"

@interface StageSceneController ()
...
@implementation StageSceneController

SCNNode *cameraNode;
double pitchRestrictss = 9;
...
- (IBAction)scrolly:(UIPanGestureRecognizer *)recognizer {
    ...
    if( RADIANS_TO_DEGREES(pitch) > pitchRestrictss ) pitch = DEGREES_TO_RADIANS(pitchRestrictss);
    else if( RADIANS_TO_DEGREES(pitch) < -pitchRestrictss ) pitch = DEGREES_TO_RADIANS(-pitchRestrictss);
...
- (void)viewDidLoad {
    ...
    // Add sphere thing
    SCNScene *geo = [SCNScene sceneNamed:@"stage2Scene.dae"];
